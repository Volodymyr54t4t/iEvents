...
async function loadCompetitionParticipants() {
    ...
    try {
        const response = await fetch(
            `http://localhost:3000/api/competitions/${currentCompetitionId}/participants-with-results`,
        )
        const data = await response.json()
...
async function saveResult() {
    ...
    try {
        ...
        if (editMode) {
            ...
            response = await fetch(`http://localhost:3000/api/results/${resultId}`, {
                method: "PUT",
                headers: {
            ...
            response = await fetch("http://localhost:3000/api/results", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
