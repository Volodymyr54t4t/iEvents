...
function generateAvatarElement(student) {
  if (student.avatar) {
    return `<img src="${student.avatar}" alt="${student.first_name || "Учень"}" class="avatar-img" onerror="this.style.display='none'; this.parentElement.textContent='${generateAvatarInitials(student.first_name, student.last_name)}'">`
  }

  return `<div class="avatar-initials">${generateAvatarInitials(student.first_name, student.last_name)}</div>`
}
