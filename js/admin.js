...
function checkAuth() {
  ...
  const allowedRoles = [
    "методист",
    "адміністратор_платформи",
    "адміністратор_громади",
    "вчитель",
  ];
...
async function loadDashboard() {
  try {
    // Load statistics
    const [usersStats, competitionsStats, resultsCount, schoolsCount] =
      await Promise.all([
        apiRequest("/api/admin/stats/users").catch(() => ({
          total: 0,
          byRole: {},
        })),
        apiRequest("/api/admin/stats/competitions").catch(() => ({
          total: 0,
          byStatus: {},
        })),
    ...
    document.getElementById("totalUsersCount").textContent =
      usersStats.total || 0;
    document.getElementById("totalCompetitionsCount").textContent =
      competitionsStats.total || 0;
    document.getElementById("totalResultsCount").textContent =
      resultsCount.total || 0;
    // Schools API returns { schools: [...] } object
    const schoolsList = Array.isArray(schoolsCount)
      ? schoolsCount
      : schoolsCount.schools || [];
    document.getElementById("totalSchoolsCount").textContent =
      schoolsList.length;
...
async function loadRecentActivity() {
  ...
  try {
    ...
    container.innerHTML =
      '<p class="loading">Не вдалося завантажити активність</p>';
  }
}
...
async function loadUsers() {
  ...
  try {
    ...
    tbody.innerHTML =
      '<tr><td colspan="7" class="loading">Помилка завантаження</td></tr>';
  }
}
...
function renderUsersTable() {
  ...
  if (!allUsersData.length) {
    tbody.innerHTML =
      '<tr><td colspan="7" class="loading">Користувачів не знайдено</td></tr>';
    return;
  }
...
async function loadCompetitions() {
  ...
  try {
    ...
    const status =
      document.getElementById("competitionStatusFilter")?.value || "";

    const response = await apiRequest("/api/competitions");
    let competitions = Array.isArray(response)
      ? response
    ...
    if (status) {
      competitions = competitions.filter(
        (c) => getCompetitionStatus(c) === status,
      );
    }
...
function renderCompetitionsTable() {
  ...
  if (!competitionsData.length) {
    tbody.innerHTML =
      '<tr><td colspan="7" class="loading">Конкурсів не знайдено</td></tr>';
    return;
  }

  const start =
    (pagination.competitions.page - 1) * pagination.competitions.limit;
  const paginatedData = competitionsData.slice(
    start,
    start + pagination.competitions.limit,
  );
...
function editCompetition(id) {
  ...
  document.getElementById("competitionModalTitle").textContent =
    "Редагувати конкурс";
  ...
  document.getElementById("competitionDescription").value =
    comp.description || "";
  document.getElementById("competitionStartDate").value = formatDateForInput(
    comp.start_date,
  );
  document.getElementById("competitionEndDate").value = formatDateForInput(
    comp.end_date,
  );
  ...
  document.getElementById("competitionMaxParticipants").value =
    comp.max_participants || "";
  document.getElementById("competitionStatus").value = comp.manual_status || "";
  document.getElementById("competitionIsOnline").checked =
...
async function saveCompetition(event) {
  ...
  const data = {
    ...
    max_participants:
      document.getElementById("competitionMaxParticipants").value || null,
...
function deleteCompetition(id) {
  confirmAction(
    "Ви впевнені, що хочете видалити цей конкурс? Всі пов'язані дані будуть втрачені.",
    async () => {
      try {
...
async function loadResults() {
  ...
  try {
    const competitionId =
      document.getElementById("resultCompetitionFilter")?.value || "";
    ...
    tbody.innerHTML =
      '<tr><td colspan="8" class="loading">Помилка завантаження</td></tr>';
  }
}
...
function renderResultsTable() {
  ...
  if (!resultsData.length) {
    tbody.innerHTML =
      '<tr><td colspan="8" class="loading">Результатів не знайдено</td></tr>';
    return;
  }
  ...
  const paginatedData = resultsData.slice(
    start,
    start + pagination.results.limit,
  );
...
async function loadSchools() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="4" class="loading">Завантаження...</td></tr>';
  ...
  try {
    ...
    tbody.innerHTML =
      '<tr><td colspan="4" class="loading">Помилка завантаження</td></tr>';
  }
}
...
function renderSchoolsTable() {
  ...
  if (!schoolsData.length) {
    tbody.innerHTML =
      '<tr><td colspan="4" class="loading">Шкіл не знайдено</td></tr>';
    return;
  }

  tbody.innerHTML = schoolsData
    .map(
      (school) => `
        <tr>
            <td>${school.id}</td>
...
function renderSubjectsTable() {
  ...
  if (!subjectsData.length) {
    tbody.innerHTML =
      '<tr><td colspan="4" class="loading">Предметів не знайдено</td></tr>';
    return;
  }

  tbody.innerHTML = subjectsData
    .map(
      (subject) => `
        <tr>
            <td>${subject.id}</td>
...
function renderNewsTable() {
  ...
  if (!newsData.length) {
    tbody.innerHTML =
      '<tr><td colspan="7" class="loading">Новин не знайдено</td></tr>';
    return;
  }
...
async function loadCompetitionsForSelect() {
  try {
    const response = await apiRequest("/api/competitions");
    const competitions = Array.isArray(response)
      ? response
      : response.competitions || [];
    ...
    if (formSelect)
      formSelect.innerHTML = options.replace("Всі конкурси", "Оберіть конкурс");
  } catch (error) {
...
function renderPagination(type, paginationData) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    if (
      i === 1 ||
      i === totalPages ||
      (i >= paginationData.page - 2 && i <= paginationData.page + 2)
    ) {
...
document.getElementById("competitionSearch")?.addEventListener(
  "input",
  debounce(() => {
...
document
  .getElementById("competitionStatusFilter")
...
document
  .getElementById("resultCompetitionFilter")
  ?.addEventListener("change", () => {
    pagination.results.page = 1;
    loadResults();
  });

document.getElementById("resultSearch")?.addEventListener(
  "input",
  debounce(() => {
    pagination.results.page = 1;
    loadResults();
...
document
  ...
  ?.addEventListener("input", debounce(loadSchools, 300));
document
  .getElementById("newsSearch")
  ?.addEventListener("input", debounce(loadNews, 300));
