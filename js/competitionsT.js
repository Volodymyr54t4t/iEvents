...
document.addEventListener("DOMContentLoaded", () => {
  if (userRole === "методист") {
    document.getElementById("createCompetitionCard").style.display = "block"
  }
      ...
      if (data.competitions.length === 0) {
        container.innerHTML = `
          <div class="empty-state">
            ...
            <p>${userRole === "методист" ? "Створіть перший конкурс за допомогою форми вище" : "Конкурси з'являться тут після їх створення методистом"}</p>
          </div>
        `
        return
      }
