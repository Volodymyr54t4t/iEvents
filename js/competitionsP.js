...
async function loadMyCompetitions() {
  try {
    const response = await fetch(`${API_URL}/api/competitions/my/${userId}`)
...
async function loadMyResults() {
  ...
  try {
    ...
    const competitionsResponse = await fetch(`${API_URL}/api/competitions/my/${userId}`)
    ...
    for (const competition of competitionsData.competitions) {
      try {
        const resultsResponse = await fetch(`${API_URL}/api/results/${competition.id}`)
