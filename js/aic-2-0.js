...
function renderStudentsList(students) {
  ...
  if (students.length === 0) {
    container.innerHTML =
      '<div class="aic-loading"><p>Учнів не знайдено</p></div>';
    return;
  }
...
function renderStudentBanner() {
  ...
  document.getElementById("bannerGrade").textContent = studentData.grade
    ? `${studentData.grade} клас`
    : "Клас не вказано";
  document.getElementById("bannerSchool").textContent =
    studentData.school || studentData.city || "Заклад не вказано";
  ...
  // Stats
  document.getElementById("bannerTotalComps").textContent =
  ...
  const places = studentResults
    .map((r) => parseInt(r.place))
  ...
  if (studentResults.length > 0) {
    ...
    const prizeRate =
      studentResults.length > 0
        ? prizeResults.length / studentResults.length
        : 0;
      ...
      Math.max(
        ...
        Math.round(
          prizeRate * 50 +
            (avgScore / 100) * 30 +
            trendBonus * 100 +
            (studentResults.length > 5 ? 10 : studentResults.length * 2),
        ),
      ),
    );

    if (probability >= 70)
      note = "Висока імовірність! Рекомендуємо конкурси вищого рівня";
    else if (probability >= 40)
      note = "Середня імовірність. Є потенціал для покращення";
...
function renderLevelRecommendation() {
  ...
  document
    ...
    .style.setProperty("--progress", `${progressPercent}%`);
...
function renderTrendChart() {
    ...
    trendChartInstance = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          title: {
            display: true,
            text: "Недостатньо даних для побудови тренду",
            color: "#8b7355",
          },
        },
      },
  ...
  const predictedData = Array(scores.length - 1)
    .fill(null)
    .concat([scores[scores.length - 1], predicted1, predicted2]);
...
function analyzeCompetencies() {
  ...
  competencyMap["Аналітичне мислення"] = Math.min(
    100,
    Math.round(avgScore * 1.1),
  );

  // Creativity: based on achievement variety and improving trend
  const uniqueAchievements = new Set(
    studentResults.map((r) => r.achievement).filter(Boolean),
  );
  competencyMap["Креативність"] = Math.min(
    100,
    Math.round(
  ...
  competencyMap["Систематичність"] = Math.min(
    100,
    Math.round(
  ...
  competencyMap["Комунікація"] = Math.min(
    100,
    Math.round(
  ...
  competencyMap["Стресостійкість"] = Math.min(
    100,
    Math.round(
      stability * 60 +
  ...
  competencyMap["Адаптивність"] = Math.min(
    100,
    Math.round(
...
function renderIndices() {
  ...
  document.getElementById("stemNote").textContent =
    stemIndex >= 70
      ? "Високий рівень STEM-компетентності"
      : stemIndex >= 40
        ? "Середній рівень, є потенціал для зростання"
  ...
  document.getElementById("improvementNote").textContent =
    improvement >= 70
      ? "Відмінна динаміка розвитку!"
      : improvement >= 40
        ? "Стабільний темп розвитку"
  ...
  document.getElementById("engagementNote").textContent =
    engagement >= 70
...
function calculateSTEMIndex(avgScore, totalComps, trend) {
  ...
  return Math.min(
    100,
    Math.round(scoreComponent + expComponent + trendComponent),
  );
}
...
function renderHistoryTable() {
  ...
  if (studentResults.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="loading">Немає даних про участь у конкурсах</td></tr>';
    return;
  }
